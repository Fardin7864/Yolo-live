-- Stable Explore rankings for both hosts and gifters.
-- diamond_cost is already the full cost of a batch gift; count is summed only
-- for display and must not be multiplied into the diamond total.

CREATE INDEX IF NOT EXISTS gifts_log_sender_created_idx
  ON public.gifts_log(sender_id, created_at DESC);
CREATE INDEX IF NOT EXISTS gifts_log_receiver_created_idx
  ON public.gifts_log(receiver_id, created_at DESC);
...
BEGIN
  ...
  IF v_kind = 'host' THEN
    ...
    WHERE gift_row.created_at >= v_start AND gift_row.created_at < v_end
      ...
      AND EXISTS (
        SELECT 1
        FROM public.live_streams AS broadcaster_stream
