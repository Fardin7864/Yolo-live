-- Self-gifting never actually worked: `self_gifting_core` patched send_gift(),
-- but the live screen sends every gift through send_gift_batch(), which
-- rejected the sender outright with "You cannot gift yourself". The feature was
-- switched on in system_settings yet gifts_log has never recorded a single
-- self-gift.
--
-- This teaches the batch path the same rules send_gift() already enforces:
--   * the Super Admin switch (system_settings.self_gifting.enabled)
--   * self-gifts only inside the host's OWN running live
--   * the Dhaka-day diamond and count ceilings
--   * is_self_gift on the log row, so rankings and reports keep excluding them
--   * beans parked in non_withdrawable_beans unless count_toward_earnings
--   * the self share left out of live_streams.total_earnings on the same rule
--
-- A batch may mix self and other recipients; only the sender's own row is
-- treated as a self-gift, and only that share counts against the daily limit.
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM batch_recipients WHERE user_id = me) INTO v_has_self;
  IF v_has_self THEN
  ...
  IF v_has_self AND (v_cfg.daily_diamond_limit > 0 OR v_cfg.daily_count_limit > 0) THEN
  ...
  IF p_room_id IS NOT NULL THEN
    ...
    SELECT CASE WHEN EXISTS(
      ...
      WHERE recipient.user_id = v_room_host_id
        AND (recipient.user_id <> me OR v_self_counts)
