-- Fix: ordinary gifts were aborting with 'record "v_cfg" is not assigned yet'.
--
-- send_gift_batch only loads v_cfg (the self-gift settings) when the sender is
-- among the recipients. The daily-ceiling check then guarded itself with
-- "IF v_has_self AND (v_cfg.daily_diamond_limit > 0 OR ...)". PL/pgSQL evaluates
-- such a condition as one SQL expression and does NOT short-circuit AND, so the
-- v_cfg fields were dereferenced even when v_has_self was false -- which is every
-- normal gift to another user. The record was unassigned, and the whole gift
-- failed.
--
-- The ceiling check is now nested inside "IF v_has_self THEN" so v_cfg is only
-- ever touched after it has been loaded. No other behaviour changes.
