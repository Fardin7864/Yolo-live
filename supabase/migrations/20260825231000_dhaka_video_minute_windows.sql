-- Use Dhaka day slices for both valid-day qualification and displayed video
-- minutes. This keeps a video session that crosses midnight in the correct
  ...
  WITH session_rows AS (
    ...
    SELECT
      ...
      (SELECT COUNT(*) FROM video_day_minutes
        WHERE day_date = (NOW() AT TIME ZONE 'Asia/Dhaka')::DATE
          AND video_minutes >= 35)::BIGINT AS today_live_days,
      COALESCE((SELECT SUM(video_minutes) FROM video_day_minutes
        WHERE day_date = (NOW() AT TIME ZONE 'Asia/Dhaka')::DATE), 0)::BIGINT AS today_minutes,
      ...
      (SELECT COUNT(*) FROM video_day_minutes
        WHERE day_date >= (NOW() AT TIME ZONE 'Asia/Dhaka')::DATE - 6
          AND video_minutes >= 35)::BIGINT AS week_live_days,
      COALESCE((SELECT SUM(video_minutes) FROM video_day_minutes
        WHERE day_date >= (NOW() AT TIME ZONE 'Asia/Dhaka')::DATE - 6), 0)::BIGINT AS week_minutes,
      COALESCE(SUM(session_row.total_gifts) FILTER (
      ...
      (SELECT COUNT(*) FROM video_day_minutes
        WHERE day_date >= (NOW() AT TIME ZONE 'Asia/Dhaka')::DATE - 29
          AND video_minutes >= 35)::BIGINT AS month_live_days,
      COALESCE((SELECT SUM(video_minutes) FROM video_day_minutes
        WHERE day_date >= (NOW() AT TIME ZONE 'Asia/Dhaka')::DATE - 29), 0)::BIGINT AS month_minutes,
      COALESCE(SUM(session_row.total_gifts) FILTER (
