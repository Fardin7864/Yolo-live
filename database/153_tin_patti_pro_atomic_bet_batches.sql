-- One idempotent transaction per player/round replaces one RPC per tap.
-- Stored bet rows remain valid 1K/5K/50K/100K chip denominations.
...
REVOKE ALL ON FUNCTION public.place_tin_patti_pro_bet_batch(UUID, TEXT, JSONB)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.place_tin_patti_pro_bet_batch(UUID, TEXT, JSONB)
  TO authenticated, service_role;

COMMENT ON FUNCTION public.place_tin_patti_pro_bet_batch(UUID, TEXT, JSONB) IS
  'Idempotent atomic boundary batch for Teen Patti Pro V2.';
