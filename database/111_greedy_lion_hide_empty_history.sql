-- =====================================================================
-- 111_greedy_lion_hide_empty_history.sql
-- Save empty Greedy Lion rounds, mark them, and hide them from history.
-- =====================================================================

UPDATE public.game_rounds
   SET result = COALESCE(result, '{}'::jsonb) || jsonb_build_object('is_empty_round', true)
 WHERE game_type = 'greedy_lion'
   AND status = 'settled'
   AND COALESCE(total_bet, 0) = 0
   AND COALESCE(result->>'is_empty_round', 'false') <> 'true';

UPDATE public.game_rounds
   SET result = COALESCE(result, '{}'::jsonb) || jsonb_build_object('is_empty_round', false)
 WHERE game_type = 'greedy_lion'
   AND status = 'settled'
   AND COALESCE(total_bet, 0) > 0
   AND result ? 'is_empty_round' IS FALSE;
  ...
  FROM (
    SELECT gr.id, gr.winner_pos, gr.result, COALESCE(gr.ends_at, gr.started_at, gr.created_at) AS settled_at
     ...
     WHERE gr.room_id = public.greedy_lion_global_room_id()
       ...
       AND COALESCE((gr.result->>'is_empty_round')::boolean, COALESCE(gr.total_bet, 0) = 0) IS DISTINCT FROM TRUE
