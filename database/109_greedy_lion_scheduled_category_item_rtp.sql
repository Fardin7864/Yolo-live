-- =====================================================================
-- 109_greedy_lion_scheduled_category_item_rtp.sql
-- Greedy Lion: scheduled Pizza/Salad categories + exact-item RTP mode.
-- =====================================================================
...
UPDATE public.game_settings
   SET special_result_rules = COALESCE(NULLIF(special_result_rules, '{}'::jsonb), '{
     "pizza_enabled": true,
     "pizza_per_hour": 0,
     "pizza_max_per_day": 0,
     "salad_enabled": true,
     ...
     "salad_max_per_day": 0
       ...
       forced_next_result = COALESCE(forced_next_result, forced_next_category)
 WHERE id = 'greedy_lion';

CREATE OR REPLACE FUNCTION public.greedy_lion_result_is_valid(p_result TEXT)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  SELECT p_result IN ('pizza', 'salad')
      OR EXISTS (
        SELECT 1
          FROM public.greedy_lion_multiplier(p_result) gm
         WHERE gm.category IS NOT NULL
      );
$$;
...
BEGIN
  ...
  v_target_percent := GREATEST(0, LEAST(100, COALESCE(v_settings.win_chance_percent, 30)));
  v_target_payout := v_total_bet * (v_target_percent / 100.0);
    ...
    IF v_winner_pos IS NULL THEN
      ...
      LOOP
        ...
        IF v_best_item IS NULL
           OR ABS(v_item.liability - v_target_payout) < v_best_diff
           OR (ABS(v_item.liability - v_target_payout) = v_best_diff AND v_item.liability < COALESCE(v_best_liability, v_item.liability + 1)) THEN
          v_best_item := v_item.id;
          v_best_liability := v_item.liability;
          v_best_diff := ABS(v_item.liability - v_target_payout);
  ...
  UPDATE public.game_rounds
     SET status = 'settled',
         ...
         result = jsonb_build_object(
           ...
           'winner_pos', v_winner_pos,
           'category', v_category,
           'target_payout_percent', v_target_percent,
           ...
           'schedule_counts', jsonb_build_object(
             'pizza_hour', v_pizza_hour_count,
             'pizza_day', v_pizza_day_count,
             'salad_hour', v_salad_hour_count,
             'salad_day', v_salad_day_count
           ),
...
GRANT EXECUTE ON FUNCTION public.greedy_lion_result_is_valid(TEXT) TO authenticated, service_role;
