-- =====================================================================
-- 120_tin_patti_pro_fix_ambiguous_id.sql
-- Replaces Tin Patti Pro RPCs with qualified column references.
-- Use this after 119 if the remote DB already has the original functions.
-- =====================================================================
...
ALTER TABLE public.game_settings
  ...
  ADD COLUMN IF NOT EXISTS forced_next_result TEXT,
  ADD COLUMN IF NOT EXISTS special_result_rules JSONB NOT NULL DEFAULT '{}'::jsonb;
...
VALUES (
  ...
  NULL,
  '[
    {"id":"crown","label":"Crown","m":2.9},
    {"id":"coffee","label":"Coffee","m":2.9},
    {"id":"cake","label":"Cake","m":2.9}
  ]'::jsonb,
  30,
  15,
  NULL,
  ...
  '{
    ...
    "bet_acceptance_grace_s": 10,
    "post_betting_block_s": 5
  }'::jsonb
)
ON CONFLICT (id) DO UPDATE SET
  multipliers = COALESCE(public.game_settings.multipliers, EXCLUDED.multipliers),
  round_duration_s = COALESCE(public.game_settings.round_duration_s, EXCLUDED.round_duration_s),
  result_display_s = COALESCE(public.game_settings.result_display_s, EXCLUDED.result_display_s),
  min_bet = COALESCE(public.game_settings.min_bet, EXCLUDED.min_bet),
  special_result_rules = COALESCE(NULLIF(public.game_settings.special_result_rules, '{}'::jsonb), EXCLUDED.special_result_rules);

CREATE OR REPLACE FUNCTION public.tin_patti_pro_global_room_id()
RETURNS UUID
LANGUAGE sql
IMMUTABLE
AS $$
...
BEGIN
  ...
  LOOP
    v_i := v_i + 1;
    id := v_item ->> 'id';
    label := COALESCE(v_item ->> 'label', v_item ->> 'id');
...
AS $$
  SELECT EXISTS (
    SELECT 1
      FROM public.tin_patti_pro_options() opt
     WHERE opt.id = p_position
  );
$$;

CREATE OR REPLACE FUNCTION public.tin_patti_pro_bet_grace_seconds()
RETURNS INT
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  IF p_winner = 'crown' THEN
    RETURN jsonb_build_object(
      'crown', jsonb_build_object('rank', 'Trail', 'rank_value', 6, 'cards', jsonb_build_array(
        jsonb_build_object('value', 'A', 'suit', 'H'),
        jsonb_build_object('value', 'A', 'suit', 'S'),
        jsonb_build_object('value', 'A', 'suit', 'D')
      )),
      'coffee', jsonb_build_object('rank', 'High Card', 'rank_value', 1, 'cards', jsonb_build_array(
        jsonb_build_object('value', 'K', 'suit', 'H'),
        jsonb_build_object('value', '9', 'suit', 'D'),
        jsonb_build_object('value', '5', 'suit', 'S')
      )),
      'cake', jsonb_build_object('rank', 'High Card', 'rank_value', 1, 'cards', jsonb_build_array(
        jsonb_build_object('value', '7', 'suit', 'D'),
        jsonb_build_object('value', '4', 'suit', 'C'),
        jsonb_build_object('value', '3', 'suit', 'S')
      ))
    );
  ELSIF p_winner = 'coffee' THEN
    RETURN jsonb_build_object(
      'crown', jsonb_build_object('rank', 'High Card', 'rank_value', 1, 'cards', jsonb_build_array(
        jsonb_build_object('value', 'K', 'suit', 'S'),
        jsonb_build_object('value', '9', 'suit', 'H'),
        jsonb_build_object('value', '5', 'suit', 'C')
      )),
      'coffee', jsonb_build_object('rank', 'Pure Sequence', 'rank_value', 5, 'cards', jsonb_build_array(
        jsonb_build_object('value', 'A', 'suit', 'S'),
        jsonb_build_object('value', 'K', 'suit', 'S'),
        jsonb_build_object('value', 'Q', 'suit', 'S')
      )),
      'cake', jsonb_build_object('rank', 'High Card', 'rank_value', 1, 'cards', jsonb_build_array(
        jsonb_build_object('value', '8', 'suit', 'D'),
        jsonb_build_object('value', '4', 'suit', 'H'),
        jsonb_build_object('value', '2', 'suit', 'C')
      ))
    );
  END IF;
  ...
  RETURN jsonb_build_object(
    'crown', jsonb_build_object('rank', 'High Card', 'rank_value', 1, 'cards', jsonb_build_array(
      jsonb_build_object('value', 'J', 'suit', 'D'),
      jsonb_build_object('value', '8', 'suit', 'S'),
      jsonb_build_object('value', '4', 'suit', 'C')
    )),
    'coffee', jsonb_build_object('rank', 'High Card', 'rank_value', 1, 'cards', jsonb_build_array(
      jsonb_build_object('value', '10', 'suit', 'H'),
      jsonb_build_object('value', '7', 'suit', 'D'),
      jsonb_build_object('value', '3', 'suit', 'S')
    )),
    'cake', jsonb_build_object('rank', 'Pair', 'rank_value', 2, 'cards', jsonb_build_array(
      jsonb_build_object('value', 'Q', 'suit', 'H'),
      jsonb_build_object('value', 'Q', 'suit', 'C'),
      jsonb_build_object('value', '9', 'suit', 'D')
    ))
  );
END $$;

CREATE OR REPLACE FUNCTION public.validate_tin_patti_pro_bet()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  v_game_type TEXT;
BEGIN
  SELECT gr.game_type INTO v_game_type FROM public.game_rounds gr WHERE gr.id = NEW.round_id;

  IF v_game_type = 'tin_patti_pro'
     AND public.tin_patti_pro_result_is_valid(NEW.position) IS DISTINCT FROM TRUE THEN
    RAISE EXCEPTION 'Invalid Tin Patti Pro position: %', NEW.position;
  END IF;

  RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS validate_tin_patti_pro_bet_trigger ON public.game_round_bets;
CREATE TRIGGER validate_tin_patti_pro_bet_trigger
BEFORE INSERT OR UPDATE OF position, round_id ON public.game_round_bets
FOR EACH ROW EXECUTE FUNCTION public.validate_tin_patti_pro_bet();
...
BEGIN
  ...
  ELSIF COALESCE(v_total_bet, 0) <= 0 THEN
    SELECT opt.id INTO v_winner_pos
      FROM public.tin_patti_pro_options() opt
     ORDER BY random()
     LIMIT 1;
  ELSE
    WITH option_totals AS (
      SELECT o.id,
             ...
             (COALESCE(SUM(b.amount), 0)::numeric * o.multiplier) AS payout,
             CASE WHEN v_total_bet > 0 THEN (COALESCE(SUM(b.amount), 0)::numeric * o.multiplier * 100 / v_total_bet) ELSE 0 END AS payout_pct
        FROM public.tin_patti_pro_options() o
        LEFT JOIN public.game_round_bets b
          ON b.round_id = v_round.id
         AND b.position = o.id
       GROUP BY o.id, o.multiplier, o.sort_order
    )
    SELECT option_totals.id INTO v_winner_pos
      FROM option_totals
     ORDER BY
       CASE
  ...
  FOR v_bet IN
    SELECT b.id, b.user_id, b.amount
      FROM public.game_round_bets
     WHERE game_round_bets.round_id = v_round.id
       AND game_round_bets.position = v_winner_pos
  LOOP
...
DECLARE
  ...
  v_display INT;
  v_grace INT;
  v_all_bets JSONB := '[]'::jsonb;
  v_my_bets JSONB := '[]'::jsonb;
  v_bet_totals JSONB := '{}'::jsonb;
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(h) ORDER BY h.settled_at DESC), '[]'::jsonb)
    ...
    FROM (
      SELECT gr.id,
       ...
       WHERE gr.room_id = public.tin_patti_pro_global_room_id()
  ...
  IF me IS NOT NULL THEN
  SELECT p.diamonds INTO v_my_balance FROM public.profiles p WHERE p.id = me;
  END IF;
  ...
  RETURN json_build_object(
    ...
    'bets', v_all_bets,
    'public_bets', v_all_bets,
    'my_bets', v_my_bets,
    'bet_totals', v_bet_totals,
    'history', v_history,
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = v_settings.house_profile_id) THEN
  ...
  VALUES
    (me, v_settings.house_profile_id, 'game_bet', 'diamond', -p_amount, v_balance, 'game_round', p_round_id, 'completed', 'Tin Patti Pro bet - losing stakes are burned');
...
GRANT EXECUTE ON FUNCTION public.tin_patti_pro_global_room_id() TO anon, authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.tin_patti_pro_bet_grace_seconds() TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.tin_patti_pro_deal_hands(TEXT) TO anon, authenticated, service_role;
