-- =====================================================================
-- 110_greedy_lion_payout_range_and_popup.sql
-- Greedy Lion: payout range targeting for exact item results.
-- =====================================================================

UPDATE public.game_settings
   SET special_result_rules = COALESCE(special_result_rules, '{}'::jsonb)
     || jsonb_build_object(
          'target_payout_min_percent', COALESCE((special_result_rules->>'target_payout_min_percent')::numeric, LEAST(COALESCE(win_chance_percent, 30), 40)),
          'target_payout_max_percent', COALESCE((special_result_rules->>'target_payout_max_percent')::numeric, GREATEST(COALESCE(win_chance_percent, 30), 40))
        )
